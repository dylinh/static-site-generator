...
def generate_page_recursive(dir_path_content, template_path, dest_dir_path):
    ...
    for entry in entries:
        if os.path.isfile(os.path.join(dir_path_content, entry)):
            generate_page(os.path.join(dir_path_content, entry), template_path, pathlib.Path(os.path.join(dest_dir_path, entry)).with_suffix(".html")) # from path, template path, dest path
        else:
